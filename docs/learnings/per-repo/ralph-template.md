---
title: "ralph-loop-cc-tdd-wt-vibe-kanban-template AGENT_LEARNINGS"
description: Mirror of AGENT_LEARNINGS.md from qte77/ralph-loop-cc-tdd-wt-vibe-kanban-template.
updated: 2026-08-24
source: https://github.com/qte77/ralph-loop-cc-tdd-wt-vibe-kanban-template/blob/main/AGENT_LEARNINGS.md
---

> Auto-aggregated by `.github/scripts/learnings-aggregator.py`.
> Source: [qte77/ralph-loop-cc-tdd-wt-vibe-kanban-template/AGENT_LEARNINGS.md](https://github.com/qte77/ralph-loop-cc-tdd-wt-vibe-kanban-template/blob/main/AGENT_LEARNINGS.md).
> Manual edits will be overwritten on the next run.

ralph/LEARNINGS.md
